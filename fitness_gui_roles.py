...
class FitnessAppGUI:
    ...
    def menu_ui(self):
        ...
        tk.Button(self.root, text="1. Вход / Регистрация", width=40, command=self.handle_registration_login).pack(pady=3)
        tk.Button(self.root, text="2. Преглед на фитнеси", width=40, command=self.handle_nearby_gyms).pack(pady=3)
        tk.Button(self.root, text="3. Любими фитнеси", width=40, command=self.handle_favorite_gyms).pack(pady=3)
        tk.Button(self.root, text="4. Търсене", width=40, command=self.search_gyms).pack(pady=3)
        tk.Button(self.root, text="5. Ревюта", width=40, command=self.handle_reviews).pack(pady=3)

        tk.Button(self.root, text="6. Администрация", width=40, command=self.admin_panel).pack(pady=3)
        tk.Button(self.root, text="Изход", width=40, command=self.root.quit).pack(pady=10)

    def show_message(self, title, msg):
        messagebox.showinfo(title, msg)

    def handle_registration_login(self):
        choice = simpledialog.askinteger("Опции", "1 - Регистрация\n2 - Вход")
        if choice == 1:
            self.register_user()
        elif choice == 2:
            self.login_user()
    ...
    def register_user(self):
        ...
        full_name = simpledialog.askstring("Пълно име (по избор):")

        if any(u.username == username for u in self.registered_users):
            self.show_message("Грешка", "Потребителското име вече съществува.")
            return

        user = User(username, email, password, role, full_name)
        self.registered_users.append(user)
        self.current_user = user
        self.show_message("Успех", f"Регистрацията е успешна като {role}.")
    ...
    def login_user(self):
        ...
        if user:
            self.current_user = user
            self.show_message("Успех", f"Добре дошли, {user.username} ({user.role})!")
        else:
    ...
    def handle_nearby_gyms(self):
        ...
        if self.current_user:
            gym_id = simpledialog.askinteger("Любими", "ID на фитнес за добавяне към любими (по избор):")
